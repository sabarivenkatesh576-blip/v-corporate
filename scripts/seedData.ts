...
async function seed() {
  ...
  for (const roleDef of CAREER_ROLES) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const diff: QuestionDifficulty = i < 5 ? 'beginner' : i < 15 ? 'intermediate' : 'advanced';
      const pId = 'PROJ-' + role.replace(/\s+/g, '').toUpperCase().slice(0, 4) + '-' + String(i + 1).padStart(3, '0');
      const tasks = [
        { taskId: 'T1', title: 'Problem Scoping & Architecture', description: 'Analyze business metrics and workflow constraints for ' + title + '.', deliverableType: 'document', expectedOutput: 'Architecture Specification Document', status: 'To Do' },
  ...
  qTopics.forEach((topic, idx) => {
    for (let i = 1; i <= 7; i++) {
      const diff: QuestionDifficulty = i <= 2 ? 'beginner' : i <= 5 ? 'intermediate' : 'advanced';
      questionDocs.push({
        category: 'Quantitative Aptitude', topic, difficulty: diff,
        question: 'In a corporate calculation involving ' + topic + ', if initial capacity is ' + (100 + i * 15) + ' units and final output reaches ' + (140 + i * 25) + ' units in ' + (2 + i) + ' cycles, what is the net percentage efficiency gain?',
  ...
  lTopics.forEach((topic, idx) => {
    for (let i = 1; i <= 8; i++) {
      const diff: QuestionDifficulty = i <= 3 ? 'beginner' : i <= 6 ? 'intermediate' : 'advanced';
      questionDocs.push({
